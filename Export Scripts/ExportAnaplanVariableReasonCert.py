#Import modules
import os
from anaplanConnector import Connection
#import secret keys from a differt module
from AnaplanCertPasscode import get_anaplan_Certcredentials # type: ignore
privateKey, publicKey, workspace_id, Operating_model_id,Revenue_model_id, Commission_model_id= get_anaplan_Certcredentials()

from AnaplanPasscode import get_anaplan_credentials
anaplan_email, anaplan_password, workspace_id, Operating_model_id,Revenue_model_id, Commission_model_id= get_anaplan_credentials()

# Insert your fileName
fileName = 'Variable Reasons.csv'
filePath = r'G:\Shared drives\Confidential.Finance\Finance.Squad\Confidential Finance\FP&A/PowerBi\1. Datasets and Uploads\Monthly Export'

# Use os.path.join() to concatenate the filePath and fileName
fullFilePath = os.path.join(filePath, fileName)

#Define connection
anaplan = Connection(authType='certificate', privateCertPath=privateKey,publicCertPath=publicKey,workspaceId=workspace_id, modelId=Revenue_model_id)
#anaplan = Connection(authType='basic', email=anaplan_email, password=anaplan_password,workspaceId=workspace_id, modelId=Revenue_model_id)

#Get export ID
#print(anaplan.getExports())

#run export command
anaplan.export(anaplan.getExportIdByName('Variable Reasons'), fullFilePath)